...
@router.delete("/delete-face/{employee_id}")
async def delete_face(employee_id: str):
    """
    Delete all stored face data (embedding file + raw images) for an employee.
    Called automatically when an employee is deleted in the backend.
    """
    try:
        result = face_service.delete_face_data(employee_id)
        return result
    except Exception as e:
        logger.error("delete_face error: %s", traceback.format_exc())
